...
class AvUpdater:
    ...
    def run(self):
        self._remove_src_files(lambda p: p.stat().st_size <=  # noqa: W504,W503
                               1024 * 1024 * 100  # 100mb
            ...
            if src.is_file():
                ...
                if new_dst.exists():
                    if os.path.samefile(src, new_dst):
                        self.log.debug(
                            f'skipping move same files: {src}, {new_dst}')
                        continue
                    else:
    ...
    def _remove_src_files(self, filter: Callable[[Path], bool]):
        ...
        for path in src.rglob('*'):
            ...
            if path.is_file() and filter(path):
                self.log.info(
                    f'removing {str(path)}, size={path.stat().st_size}')
                os.remove(path)
            else:
    ...
    def _docker_attach_check(self, name):
        ...
        if res['StatusCode'] != 0:
            raise Exception(
                f"container {name} exit failed {res['StatusCode']}")
    ...
    def _start_mdc(self):
        ...
        try:
            ...
            if created_vol:
                self.log.info(
                    f'starting new mdc for new volume {volname} at first')
                c.start()
...
if __name__ == "__main__":
    logging.basicConfig(format='%(asctime)s.%(msecs)03d [%(levelname)-8s] [%(name)s.%(funcName)s]: %(message)s',
                        level=logging.INFO,
                        datefmt='%Y-%m-%d %H:%M:%S')
    mdc = MdcData('/mnt/share/Downloads/completed/avs',
